...
class BaseSyncService(abc.ABC):
    """
    Базовый синхронный сервис
    """

    def __init__(self, retry_on_statuses: set[int] | None = DEFAULT_RETRY_ON_STATUSES,
                 request_preproc: list[transformers.Transformer] | None = None,
                 response_preproc: list[transformers.Transformer] | None = None,
                 transport: transports.BaseSyncTransport | None = None):
        """ Сервис """
        self.retry_on_statuses = retry_on_statuses
        self.request_preproc = request_preproc
        self.response_preproc = response_preproc
    ...
    def send_request(self, *args, **kwargs) -> transports.ServiceResponse:
        """
        Вызов внешнего сервиса
        """
        try:
            args, kwargs = self._before_send(*args, **kwargs)
            logger.debug('Send request %s; %s', args, kwargs)
            service_response = self._send(**kwargs)
            service_response = self._transform_response(service_response)
            logger.debug('Response: %s, %s', service_response.status, service_response.response_data)
            ...
            return self._on_error(ex, *args, **kwargs)

    def _transform_response(self, response: transports.ServiceResponse) -> transports.ServiceResponse:
        """ Преобразование ответа для возврата пользователю """
        if self.response_preproc:
            for response_preproc in self.response_preproc:
                response = response_preproc.modify(response)
        return response
    ...
    def _before_send(self, *args, **kwargs):
        ...
        if self.request_preproc:
            for request_preproc in self.request_preproc:
                args, kwargs = request_preproc.modify(*args, **kwargs)
        return args, kwargs

    def _on_error(self, ex: Exception, *args, **kwargs) -> transports.ServiceResponse:
        """
        Действие на возникновение ошибки.
        """
        logger.error('Error.\nargs: %s\nkwargs: %s', pprint.pformat(args), pprint.pformat(kwargs))
...
class SyncHttpService(BaseSyncService):
    """
    Синхронный вызов сервиса по протоколу http.
    """
